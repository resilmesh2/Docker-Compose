    ...
    if [ ${#COMPONENTS_TO_UPDATE[@]} -gt 0 ]; then
        ...
        if [[ " ${COMPONENTS_TO_UPDATE[*]} " == *" Threat-Awareness_IoB "* ]]; then
            ...
            if [[ "$answer_iob1" == "y" || "$answer_iob1" == "Y" ]]; then
                echo -e "Enter your peer's URL (<IP>:<PORT>):"
                read peer_url
            ...
            UPDATE_SUMMARY+="\n- IoB (Threat Awareness): Peer feature added, integrations and dependencies updated, and UI rebuilt.\n"
        ...
        UPDATE_SUMMARY+="\n- No components from the selected deployment ($DEPLOYMENT) were affected by this update.\n"
    fi
...
if [[ -n "$UPDATE_SUMMARY" ]]; then
    ...
    {
        echo -e "\n================== UPDATE SUMMARY ($CURRENT_DATE) =================="
        echo -e "\nVersion Update: $CURRENT_VERSION -> $LATEST_VERSION\n"
