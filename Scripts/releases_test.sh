        ...
        if [ ${#COMPONENTS_TO_UPDATE[@]} -eq 0 ]; then
            ...
            if [[ " ${COMPONENTS_TO_UPDATE[*]} " == *" Threat-Awareness_PPCTI "* ]]; then
                ...
                if [ -f "$PPCTI_DIR/.env.example" ]; then
                    ...
                    # Plan A: Extracción automatizada de la clave de MISP desde output_summary.txt
                    if [ -f "$SUMMARY_FILE" ]; then
                        MISP_KEY_DETECTED=$(grep "MISP Server Authkey autogenerated:" "$SUMMARY_FILE" | awk -F': ' '{print $2}' | tr -d '[:space:]')
                    fi

                    # Plan B: Intentar leer directamente desde el archivo .env físico de MISP (Corregido para el editor de código)
                    if [[ -z "$MISP_KEY_DETECTED" && -f "$DOCKER_BASE_PATH/Threat-Awareness/MISP_Server-docker/.env" ]]; then
                        MISP_KEY_DETECTED=$(grep -i "AUTH_KEY" "$DOCKER_BASE_PATH/Threat-Awareness/MISP_Server-docker/.env" | cut -d'=' -f2 | sed "s/['\"]//g" | tr -d '[:space:]')
                    fi

                    # Plan C: Pedir de forma interactiva por terminal si los métodos automáticos fallaron
                    if [[ -z "$MISP_KEY_DETECTED" ]]; then
                        echo -e "\n⚠️ No se encontró el Authkey automático de MISP en el sistema."
                        read -p "Por favor, introduce el MISP API KEY manualmente para PPCTI: " MISP_KEY_DETECTED
                    fi
